import java.util.Random;
import java.util.concurrent.ExecutionException;
import java.util.concurrent.ForkJoinPool;
import java.util.concurrent.RecursiveTask;

import tree.TreeNode;

public class Program {

	static Random rnd = new Random();
	static int total;
	
	public static void creatRandomTree(TreeNode node, int level)
	{
		node.left=new TreeNode();
		node.right=new TreeNode();
		node.weight=rnd.nextInt(100);
		total+=node.weight;
		level--;
		if(level==0)
		{
			node.left.weight=rnd.nextInt(100);
			node.right.weight=rnd.nextInt(100);
			total+=node.left.weight;
			total+=node.right.weight;
			return;
		}
		creatRandomTree(node.left, level);
		creatRandomTree(node.right, level);
	}
	
	public static int weightTree(TreeNode root)
	{
		return
				root.weight+
				(root.left!=null?weightTree(root.left):0)+ // Если вес левой ветки не равен null, то вызываем эту же ф-ю, иначе подставляем значение 0.
				(root.right!=null?weightTree(root.right):0);
	}
	
	public static int weightTreeMulti(TreeNode root) throws InterruptedException, ExecutionException
	{
		class WeightCounter extends RecursiveTask<Integer> // Возвращает  result-bearing ForkJoinTask. 
		{
			TreeNode r;
			int level;
			public WeightCounter(TreeNode n, int level)
			{
				this.r=n;
				this.level=level;
			}
			@Override
			protected Integer compute() {
				final int newLevel=level-1;
				if(level<=0) return weightTree(r);
				
				int sum = r.weight;
				WeightCounter w1 = null, w2 = null;
				if(r.left!=null)
				{
					w1=new WeightCounter(r.left,newLevel);
					w1.fork();
				}
				if(r.right!=null)
				{
					w2=new WeightCounter(r.right,newLevel);
					w2.fork(); // выполняет ответвление и наша задача будет выполняться параллельно
				}
				if(w1!=null)
					sum+=w1.join(); // дожидается выполнения подзадачи
				if(w2!=null)
					sum+=w2.join();
				return sum;
			}
		}
		
		// Можно создать свой ForkJoinPool()
		/*return (new ForkJoinPool().
				invoke(
						new WeightCounter(root,)Runtime.getRuntime().availableProcessors()
						)));*/
		
		// Можно использовать уже существующий, который создавала JVM
		return ForkJoinPool.commonPool().invoke(
				new WeightCounter(root,
						Runtime.getRuntime().availableProcessors()));
	}
	
	public static void main(String[] args) throws InterruptedException, ExecutionException {
		
		int treeLevel=22; // 2^(n+1)-1
		if(args.length>=1)
			treeLevel=Integer.parseInt(args[0]);
		
		System.out.printf("Starting tree creating with depth %d ... \n",treeLevel);
		TreeNode root = new TreeNode();
		creatRandomTree(root,treeLevel);
		System.out.printf("Tree created with total weight %d\n",total);
		weightTree(root); weightTree(root); //PROFILE Для более точного замера времени нужно вызвать функцию раз 10 так как JVM оптимизирует код и по началу может считать дольше чем потом.
		
		long t1=System.currentTimeMillis();
		int r1=weightTree(root);
		long t2=System.currentTimeMillis();
		System.out.printf("Single weight: %d Time %d\n",r1, t2-t1);
		
		
		// Распараллеленая задача
		weightTreeMulti(root); weightTreeMulti(root);//PROFILE Для более точного замера времени нужно вызвать функцию раз 10 так как JVM оптимизирует код и по началу может считать дольше чем потом.
		
		long t3 = System.currentTimeMillis();
		int r2=weightTreeMulti(root);
		long t4 = System.currentTimeMillis();
		System.out.printf("Multi weight: %d Time %d\n",r2, t4-t3);
		
	}

}
